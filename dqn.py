...
class Trainer:
    ...
    def _save_checkpoint(self, epoch, num_episodes):
        ckp = self.policy_net.module.state_dict()
        PATH = "checkpoint_" + str(num_episodes) + ".pt"
        torch.save(ckp, PATH)
...
def write_csv(losses, gpu_id, total_epochs):
    path = Path('run_seq_' + str(gpu_id) + "_" + str(total_epochs) + '.csv')
    for loss in losses:
        inp = {"losses": loss}
        df = ps.DataFrame([inp])
...
def main(rank: int, world_size: int, save_every: int, total_epochs: int, batch_size: int):
    ...
    write_csv(losses, gpu_id, total_epochs)
...
if __name__ == "__main__":
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    flag = time.time()
    main(device, world_size, args.save_every, args.total_epochs, args.batch_size)
